...
const Filter = ({ filters, otherClasses, containerClasses, jobFilter = false }: Props) => {
      ...
      <Select onValueChange={handleUpdateParams} defaultValue={paramFilter || undefined}>
        <SelectTrigger className={`${otherClasses} body-regular light-border background-light800_dark300 text-dark500_light700 border px-5 py-2.5`}>
          <div className="line-clamp-1 flex-1 text-left">
            <SelectValue placeholder={jobFilter ? "Select Location" : "Bộ lọc"} />
          </div>
        </SelectTrigger>
        <SelectContent className={`text-dark500_light700 small-regular border-none bg-light-900 dark:bg-dark-300 ${jobFilter && "max-h-48 overflow-y-auto"}`}>
          <SelectGroup>
            {filters.map((filter) => (
