...
const LocalSearch = ({ route, iconPosition, imgSrc, placeholder, otherClasses }: CustomInputProps) => {
  ...
  return (
    <div className={`background-light800_darkgradient flex min-h-[40px] grow items-center gap-1 rounded-[10px] px-4 ${otherClasses}`}>
      {iconPosition === "left" && (
        <Image src={imgSrc} alt="search icon" width={18} height={18} className="cursor-pointer" />
      )}
      <Input
        type="text"
